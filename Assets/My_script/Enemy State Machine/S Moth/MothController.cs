...
public class MothController : MonoBehaviour
{
    ...
    private MothBaseState currentState;
    public MothPatrolState patrolState = new();
    public MothChasingState chasingState = new();
    public MothAttackState attackState = new();


    private void Awake()
    {
        rd = GetComponentInParent<Rigidbody2D>();
        attackRange.radius = statOfUnit.unitStats[7].attackRange;
    ...
    public void SwitchState(MothBaseState state)
    {
        currentState = state;
        currentState.EnterState(this);
    }
    ...
    IEnumerator CoolDownKnockBack(MothController moth)
    {
